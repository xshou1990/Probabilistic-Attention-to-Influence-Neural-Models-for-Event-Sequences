...
def train_epoch(model, training_data, optimizer, opt, prior):
    ...
    for batch in tqdm(training_data, mininterval=2,
        ...
        """ backward """
        # negative log-likelihood given influence vector 
        event_loss_ave = 0
...
def train(model, training_data, validation_data, test_data, optimizer, scheduler, opt, prior, event_interest):
    """ Start training. """
...
#     opt.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    """ train each model @ each label of interest """
    for event_interest in [119, 152, 134, 78, 75, 149, 117, 135, 170, 16, 91, 145, 35, 6, 164]:

        best_model = train(model, trainloader, devloader, testloader, optimizer, scheduler, opt, prior , event_interest)
