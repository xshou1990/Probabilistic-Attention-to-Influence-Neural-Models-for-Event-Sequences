...
def train_epoch(model, training_data, optimizer, opt, prior):
    ...
    for batch in tqdm(training_data, mininterval=2,
        ...
        """ backward """
        # negative log-likelihood given influence V
        event_loss_ave = 0
...
#             print(num_iter)
            ...
            """ compute loss """
            # negative log-likelihood conditioned on sample V
            event_ll_ave = 0
            for i in range(len(output)):
...
def train(model, training_data, validation_data,  optimizer, scheduler, opt, prior, event_interest,threshold):
    """ Start training. """
...
#     opt.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for threshold in [0.4,0.5,0.6]:
        for event_interest in [ 6,37 ]:

            best_model = train(model, trainloader, devloader, optimizer, scheduler, opt, prior , event_interest, threshold)
            ...
            with open(opt.log, 'a') as f:
                f.write(' {threshold},{event_interest},{val_ll},{train} \n'
                        .format(threshold = threshold, event_interest = opt.event_interest, val_ll= valid_ll, train =posterior_train.cpu().numpy()))
