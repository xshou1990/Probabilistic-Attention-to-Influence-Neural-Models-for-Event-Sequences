...
class Transformer(nn.Module):
    ...
    def attention_from_relation(self, relation, event_type, num_types):
        """ attention indicator: attention (score) is only given to influencing event(s)  """
    ...
    def relation_from_attention(self, attn_weights, event_type, event_time, num_types, decay_rate):
        """ maps attention scores to influence matrix """
    ...
    def forward(self, event_type, event_time, num_samples, decay_rate, event_interest, threshold ):
        """
        Return the hidden representations from decoder.
        Input: event_type: batch*seq_len;
               event_time: batch*seq_len. (time index of positions of events)
        Output: output: batch*seq_len*model_dim;
                binrel: binarized influence vetor P(V_ij=0), P(V_ij=1)  (num_types) x 2 
                influence matrix: encoding P(V_ij=1) for each entry V_ij  (num_typesxnum_types)
