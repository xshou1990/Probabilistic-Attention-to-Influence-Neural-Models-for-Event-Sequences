...
#     opt.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    """ train each model @ each threshold @ specified event of interest """
    for threshold in [0.1,0.2,0.3]:
        for event_interest in [119, 152, 134, 78, 75, 149, 117, 135, 170, 16, 91, 145, 35, 6, 164]:
            ...
            with open(opt.log, 'a') as f:
                f.write('{threshold}, {event_interest}, {valid: 8.5f}, {test: 8.5f} \n'
                        .format(threshold = threshold, event_interest = event_interest, valid=valid_ll, test=test_ll))
