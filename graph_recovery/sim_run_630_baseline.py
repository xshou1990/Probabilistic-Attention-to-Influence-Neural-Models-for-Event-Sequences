...
def eval_epoch(model, validation_data, opt, event_interest):
    ...
    with torch.no_grad():
        for batch in tqdm(validation_data, mininterval=2,
            ...
            output, attn_weights = model(event_type_0 )
...
def train(model, training_data, validation_data, test_data, optimizer, scheduler, opt, event_interest):
    ...
    for epoch_i in range(opt.epoch):
        ...
        valid_ll,_ = eval_epoch(model, validation_data, opt, event_interest )
        ...
        test_ll,_ = eval_epoch(model, test_data, opt, event_interest )
...
parsed_args.event_interest = 2
parsed_args.log='eventB_630.txt'  
...
for seed_i in range(100):
    ...
    _,train_attn = eval_epoch(model, trainloader, opt, opt.event_interest)

    valid_ll,valid_attn = eval_epoch(model, devloader, opt, opt.event_interest)

    _,test_attn = eval_epoch(model, testloader, opt, opt.event_interest)
    ...
    with open(opt.log, 'a') as f:
        f.write(' {event_interest}, {val_ll}, {train},  {valid}, {test} \n'
                .format(event_interest = opt.event_interest, val_ll= valid_ll, train =train_attn.cpu().numpy(), valid=valid_attn.cpu().numpy(), test=test_attn.cpu().numpy()))
